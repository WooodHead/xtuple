...
SELECT cmitem.*, itemsite_item_id AS item_id,
       ...
       COALESCE((cmitem_unitprice / cmitem_price_invuomratio), xsalep(0)) AS unitprice,
       COALESCE((cmitem_qtycredit * cmitem_qty_invuomratio) *
                (cmitem_unitprice / cmitem_price_invuomratio), xsalep(0)) AS extprice,
       currToBase(cmhead_curr_id,
                  COALESCE((cmitem_qtycredit * cmitem_qty_invuomratio) *
                           (cmitem_unitprice / cmitem_price_invuomratio), xsalep(0)),
                  cmhead_docdate) AS baseextprice,
       ( SELECT COALESCE(SUM(taxhist_tax), xmoney(0))
         FROM cmitemtax
