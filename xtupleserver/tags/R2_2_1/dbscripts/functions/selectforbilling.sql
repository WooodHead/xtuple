...
-- the total un-invoiced shipped amount
  ...
  IF (FOUND) THEN
	...
	cobill_taxtype_id=ptaxtypeid,	cobill_tax_id=ptaxid,
        cobill_tax_ratea = calculateTax(ptaxid, round(pQty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''A''),
        cobill_tax_rateb = calculateTax(ptaxid, round(pQty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''B''),
        cobill_tax_ratec = calculateTax(ptaxid, round(pQty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''C'')
  ...
  ELSE
    ...
    ( _cobillid, _r.coitem_id, _r.cobmisc_id,
      ...
      calculateTax(ptaxid, round(pQty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''A''),
      calculateTax(ptaxid, round(pQty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''B''),
      calculateTax(ptaxid, round(pQty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''C'') );
  END IF;
