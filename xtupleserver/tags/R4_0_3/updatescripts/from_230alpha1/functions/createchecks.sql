...
BEGIN
  ...
  SELECT bankaccnt_curr_id INTO _check_curr_id
    FROM bankaccnt
  ...
  FOR _v IN SELECT DISTINCT vend_id, vend_number, vend_name,
    ...
    IF ((SELECT SUM(currToCurr(apselect_curr_id, _check_curr_id,
			       apselect_amount, pCheckDate))
	...
	WHERE (apselect_id=_r.apselect_id);
      ...
      UPDATE checkhead
      SET checkhead_amount = (SELECT SUM(currToCurr(checkitem_curr_id,
						    _check_curr_id,
						    checkitem_amount, pCheckDate))
			      FROM checkitem
