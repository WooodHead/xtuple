...
--  Grab all of the uninvoiced coship records
  FOR _r IN SELECT cohead_id, coitem_id, SUM(coship_qty) AS qty,
                   coitem_price, item_invpricerat, item_id,
            ...
            GROUP BY cohead_id, coitem_id, cust_partialship,
                     ...
                     coitem_price, item_invpricerat, item_id LOOP
    ...
    IF (FOUND) THEN
      UPDATE cobill
         SET cobill_selectdate = CURRENT_DATE,
             ...
             cobill_tax_ratea = calculateTax(cobill_tax_id, round((cobill_qty + _r.qty) * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''A''),
             cobill_tax_rateb = calculateTax(cobill_tax_id, round((cobill_qty + _r.qty) * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''B''),
             cobill_tax_ratec = calculateTax(cobill_tax_id, round((cobill_qty + _r.qty) * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''C'')
...
--  Now insert the cobill line
      ...
      ( _cobmiscid, _r.coitem_id,
        ...
        calculateTax(_taxid, round(_r.qty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''A''),
        calculateTax(_taxid, round(_r.qty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''B''),
        calculateTax(_taxid, round(_r.qty * _r.coitem_price / _r.item_invpricerat, 2), 0.0, ''C'') );
    END IF;
