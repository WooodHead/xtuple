...
INSERT INTO uom (uom_name) SELECT DISTINCT UPPER(item_invuom) FROM item EXCEPT SELECT DISTINCT uom_name FROM uom;
INSERT INTO uom (uom_name) SELECT DISTINCT UPPER(item_priceuom) FROM item WHERE item_invpricerat!=1.0 EXCEPT SELECT DISTINCT uom_name FROM uom;
INSERT INTO uom (uom_name) SELECT DISTINCT UPPER(item_capuom) FROM item WHERE item_capinvrat!=1.0 EXCEPT SELECT DISTINCT uom_name FROM uom;
INSERT INTO uom (uom_name) SELECT DISTINCT UPPER(item_altcapuom) FROM item WHERE item_altcapinvrat!=1.0 EXCEPT SELECT DISTINCT uom_name FROM uom;
INSERT INTO uom (uom_name) SELECT DISTINCT UPPER(item_shipuom) FROM item WHERE item_shipinvrat!=1.0 EXCEPT SELECT DISTINCT uom_name FROM uom;
...
UPDATE item SET item_inv_uom_id=uom_id FROM uom WHERE (UPPER(item_invuom)=uom_name);
...
UPDATE item SET item_price_uom_id=uom_id FROM uom WHERE ((UPPER(item_priceuom)=uom_name) AND (item_invpricerat!=1.0));
...
BEGIN
  ...
  FOR _item IN SELECT item_id, item_inv_uom_id, uom_name, item_fractional,
    ...
    IF(_item.item_capinvrat!=1.0 AND UPPER(_item.item_capuom)!=_item.uom_name) THEN
      SELECT uom_id INTO _uomid FROM uom WHERE (uom_name=UPPER(_item.item_capuom));
    ...
    IF(_item.item_altcapinvrat!=1.0 AND UPPER(_item.item_altcapuom)!=_item.uom_name) THEN
      SELECT uom_id INTO _uomid FROM uom WHERE (uom_name=UPPER(_item.item_altcapuom));
      IF(_uomid IS NULL) THEN
        RAISE EXCEPTION ''There was an error looking up a UOM.'';
      END IF;
    ...
    IF(_item.item_shipinvrat!=1.0 AND UPPER(_item.item_shipuom)!=_item.uom_name) THEN
      SELECT uom_id INTO _uomid FROM uom WHERE (uom_name=UPPER(_item.item_shipuom));
      IF(_uomid IS NULL) THEN
        RAISE EXCEPTION ''There was an error looking up a UOM.'';
      END IF;
