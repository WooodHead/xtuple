...
BEGIN
  ...
  ELSIF (TG_OP = 'INSERT' ) THEN
    IF (NEW.emp_username IS NULL) THEN
      SELECT usr_username INTO NEW.emp_username
      FROM usr WHERE (usr_username=NEW.emp_code);
    END IF;
  ELSIF (TG_OP = 'UPDATE') THEN
    IF (OLD.emp_code != NEW.emp_code AND NEW.emp_username IS NULL) THEN
      SELECT usr_username INTO NEW.emp_username
      FROM usr WHERE (usr_username=NEW.emp_code);
    END IF;
  END IF;
...
BEGIN
  IF (TG_OP = 'INSERT') THEN
    SELECT usr_username INTO _username FROM usr WHERE (usr_username=NEW.emp_code);
