...
SELECT cohead_id AS orderid, -2 AS itemid,
       ...
       '' AS qtytobill,
       '' AS price,
       formatMoney( SUM(round(cobill_qty * coitem_price / iteminvpricerat(item_id),2)) +
...
UNION SELECT cohead_id AS orderid, coitem_id AS itemid,
             ...
             formatMoney(round(cobill_qty * coitem_price / iteminvpricerat(item_id),2)) AS extprice,
             'Credit' AS sence,
             COALESCE( ( SELECT formatGLAccountLong(accnt_id)
