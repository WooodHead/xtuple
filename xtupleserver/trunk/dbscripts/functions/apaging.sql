...
BEGIN
  ...
  FOR _x IN
        ...
        formatMoney(CASE WHEN((apopen_duedate >= DATE(pAsOfDate)))
        ...
        THEN (currtobase(apopen_curr_id,(apopen_amount - apopen_paid),apopen_docdate) *
        ...
        formatMoney(CASE WHEN((apopen_duedate > DATE(pAsOfDate)-10000))
        THEN ((currtobase(apopen_curr_id,apopen_amount,apopen_docdate)-apapplied(apopen_id,pAsOfDate)) *
        ...
        formatMoney(CASE WHEN apopen_doctype IN ('C', 'R')
        THEN currtobase(apopen_curr_id,(apopen_amount * -1),apopen_docdate) ELSE currtobase(apopen_curr_id,apopen_amount,apopen_docdate) END) AS f_apopen_amount,

        --Discount AMT base
        formatMoney(CASE WHEN((DATE(pAsOfDate) <= apopen_docdate  + terms_discdays))
