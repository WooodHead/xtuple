...
--if new sheet, do an insert to te.timehead, else, update or insert te.timedtl
	IF (pHeadID IS NULL) THEN
		...
		insert into  te.tehead(tehead_id,tehead_site,tehead_weekending,tehead_number,tehead_notes) VALUES (_headid,pSite,pWeekEnding,_sheetid,pSheetNotes);
	ELSE
		update te.tehead set tehead_lastupdated = ('now'::text)::timestamp(6) with time zone,tehead_username = current_user,tehead_notes = pSheetNotes where tehead_id = pHeadID;
	END IF;
