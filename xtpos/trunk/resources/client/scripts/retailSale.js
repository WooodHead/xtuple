...
function cancel()
{
  ...
  _sale.setFilter("closed=false AND terminal='" + _terminal.code + "'");
  _sale.select();
  mywindow.close();
}
...
{
  var childwnd = toolbox.openWindow("retailCustomer", mywindow, Qt.WindowModal, Qt.Window);
...
function payment()
{
  ...
  try
  {
    _notes.plainText = _notes2.plainText;
    _notes.updateMapperData(); // 8447
...
function populateTaxzone()
{
  var params = new Object;
  if ( metrics.value("RetailUseCustTaxZone") == "t")
    params.cust_id = _cust.id();
  else
...
function receiptSearch()
{
  ...
  // Look for a receipt
  var childwnd = toolbox.openWindow("retailSaleSearch", mywindow, Qt.WindowModal, Qt.Window);
...
function set(input)
{
  ...
  if ("index" in input)
  {
    _sale.setCurrentIndex(input.index);
    populate();
  }
