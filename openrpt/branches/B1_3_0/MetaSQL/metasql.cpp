...
class MetaSQLQueryPrivate { 
    public:
        MetaSQLQueryPrivate() {
            ...
            _top = 0;
            _log = QString::null;
            _logger = new QTextStream(&_log, IO_WriteOnly);
        }
                    ...
                    case FunctionIsLast:
                        ...
                        if(found) {
                            if(t.type() == QVariant::List || t.type() == QVariant::StringList) {
                                ...
                                QValueList<QVariant> l = t.toList();
                                if(l.size() > 0) {
                                    if((_func == FunctionIsFirst) && (pos == 0)) val = _trueVariant;
                                    else if((_func == FunctionIsLast) && ((unsigned int)(pos + 1) == l.size())) val = _trueVariant;
                                }
                            } else {
                                val = _trueVariant;
                            }
                ...
                switch(_block) {
                    ...
                    case BlockElseIf:
                        ...
                        if(!options.isEmpty()) {
                            ...
                            if(qc == '(') enclosed = true;
                            working = !enclosed;
                            for(unsigned int p = 0; p < options.length(); p++) {
                                qc = options.at(p);
            ...
            switch(_block) {
                ...
                case BlockElseIf:
                    ...
                    if(b) {
                        ...
                        for(output = _items.first(); output; output = _items.next()) {
...
bool MetaSQLQueryPrivate::populate(XSqlQuery & qry, const ParameterList & params) {
    ...
    if(_top) {
        ...
        res = qry.prepare(sql);
        for(unsigned int i = 0; i < mif._pList.count(); i++) {
...
XSqlQuery MetaSQLQuery::toQuery(const ParameterList & params, QSqlDatabase * pDb) {
    XSqlQuery qry(pDb);
    if(isValid()) {
