...
void ReportHandler::color()
{
    ...
    for(int i = 0; i < list.count(); i++)
    {
        QAbstractGraphicsShapeItem * gs = dynamic_cast<QAbstractGraphicsShapeItem*>(list.at(i));
        if(gs) {
            QPen p = gs->pen();
			if(i==0) {
            ...
            QGraphicsLineItem * l = dynamic_cast<QGraphicsLineItem*>(list.at(i));
			if(l) {
				QPen p = l->pen();
				if(i==0) {
					selectedColor = QColorDialog::getColor (p.color(), gw);
...
void ReportHandler::fill()
{
    ...
    for(int i = 0; i < list.count(); i++)
    {
        QAbstractGraphicsShapeItem * item = dynamic_cast<QAbstractGraphicsShapeItem*>(list.at(i));
        if(item) {
			if(i==0) {
