...
int code128Index(char code, int set) {
    for(int idx = 0; _128codes[idx]._null == false; idx++) {
        if(set == SETA && _128codes[idx].codea == code) return idx;
        if(set == SETB && _128codes[idx].codeb == code) return idx;
        if(set == SETC && _128codes[idx].codec == code) return idx;
    }
    return -1;  // couldn't find it
}

void renderCode128(const QRect & r, const QString & _str, int align, QPainter * pPainter) {
    QValueVector<int> str;
    unsigned int i = 0;
    ...
    if(_str.isEmpty()) {
        ...
        unsigned int rank_a = 0;
        unsigned int rank_b = 0;
        unsigned int rank_c = 0;
