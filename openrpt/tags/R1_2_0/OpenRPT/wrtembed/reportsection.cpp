...
ReportSection::ReportSection(ReportWindow * rptwin, QWidget * parent, const char * name)
  : QVBox(parent,name) {
    ...
    title = new QLabel(tr("SECTION TITLE"),this,"section title");
    title->setFrameStyle(QFrame::StyledPanel | QFrame::Raised);
...
void ReportSectionDetail::insertSection(int idx, ReportSectionDetailGroup * rsd) {
    ...
    rsd->getGroupHead()->reparent(this, QPoint());
    rsd->getGroupFoot()->reparent(this, QPoint());
...
ReportCanvasView::ReportCanvasView(ReportWindow * rw, QCanvas * canvas, QWidget * parent, const char * name)
  : QCanvasView(parent, name) {
...
//
ReportResizeBar::ReportResizeBar(QWidget * parent, const char * name, WFlags f)
...
ReportResizeBar::~ReportResizeBar() {
    // Qt should be handling everthing for us
}
