...
{
  QStringList files = QFileDialog::getOpenFileNames(tr("Report Definitions (*.xml)"),
...
{
  QListBoxItem * item = _reports->firstItem();
  QListBoxItem * next = 0;
  while(item)
  {
    next = item->next();
...
void ImportWindow::reportsDoubleClicked( QListBoxItem * item )
{
