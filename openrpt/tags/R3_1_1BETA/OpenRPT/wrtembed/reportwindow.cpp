...
bool ReportWindow::saveAs() {
    ...
    // but for now...
	QString file = QFileDialog::getSaveFileName( this, tr("Choose filename to save"), "", tr("XML (*.xml)") );
...
bool ReportWindow::saveToDb() {
    ...
    if(rptDiag.exec() == QDialog::Accepted) {
        ...
        q.prepare(getSqlFromTag("fmt09", QSqlDatabase::database().driverName()));	// MANU
        q.bindValue(":report_name", name);
        q.bindValue(":report_grade", grade);
        q.exec();
        if(q.first()) {
            // update old record
            qry.prepare(getSqlFromTag("fmt10", QSqlDatabase::database().driverName()));	// MANU
            ...
            // insert new record
            qry.prepare(getSqlFromTag("fmt11", QSqlDatabase::database().driverName()));	// MANU
            qry.bindValue(":report_name", name);
            qry.bindValue(":report_desc", desc);
            qry.bindValue(":report_src", src);
...
void ReportWindow::addDefinedParameter(const QString & n, const ORParameter & d) {
