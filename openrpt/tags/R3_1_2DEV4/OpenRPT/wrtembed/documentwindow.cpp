...
DocumentWindow::DocumentWindow(ReportGridOptions * rgo, QWidget * parent)
  : QMainWindow(parent)
{
  ...
  _scene = new DocumentScene(0, this);
  ...
  DocumentView * _view = new DocumentView(_scene, this);
...
void DocumentWindow::setCaption()
{
  ...
  if(handler()) 
  {
      if(handler()->databaseElt().hasChildNodes()) 
          cap += " (m)";
  }
