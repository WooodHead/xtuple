...
# uncomment the following line to enable the old style report window
#CONFIG += oldcode

oldcode:DEFINES += _INCLUDE_OLDCODE
DESTDIR = ../../lib
MOC_DIR = tmp
UI_DIR = tmp
OBJECTS_DIR = tmp
QT += xml sql
oldcode:QT += qt3support
#VERSION = 0.1.0
...
oldcode:HEADERS += reportentities.h\
                   reportsection.h\
                   reportwindow.h
...
oldcode:SOURCES += reportentities.cpp\
                   reportsection.cpp\
                   reportwindow.cpp\
...
TRANSLATIONS    = wrtembed_fr.ts wrtembed_it.ts wrtembed_ru.ts
