...
bool parseReportRect(const QDomElement & elemSource, ORRectData & rectTarget)
{
    ...
    for ( int paramCounter = 0; paramCounter < params.count(); paramCounter++ )
    {
        ...
        {
            rectTarget.x = elemParam.text().toDouble();
            coorCounter++;
        }
        else if (elemParam.tagName() == "y")
        {
            rectTarget.y = elemParam.text().toDouble();
            coorCounter++;
        }
        else if (elemParam.tagName() == "width")
        {
            rectTarget.width = elemParam.text().toDouble();
            coorCounter++;
        }
        else if (elemParam.tagName() == "height")
        {
            rectTarget.height = elemParam.text().toDouble();
            coorCounter++;
        }
...
bool parseReportLine(const QDomElement & elemSource, ORLineData & lineTarget)
{
  ...
  for ( int paramCounter = 0; paramCounter < params.count(); paramCounter++ )
  {
    ...
    {
      lineTarget.xStart = elemParam.text().toDouble();
      coorCounter++;
    }
    else if (elemParam.tagName() == "ystart")
    {
      lineTarget.yStart = elemParam.text().toDouble();
      coorCounter++;
    }
    else if (elemParam.tagName() == "xend")
    {
      lineTarget.xEnd = elemParam.text().toDouble();
      coorCounter++;
    }
    else if (elemParam.tagName() == "yend")
    {
      lineTarget.yEnd = elemParam.text().toDouble();
      coorCounter++;
    }
    else if (elemParam.tagName() == "weight")
...
bool parseReport(const QDomElement & elemSource, ORReportData & reportTarget)
{
  ...
  for(int nodeCounter = 0; nodeCounter < section.count(); nodeCounter++)
  {
    ...
    else if(elemThis.tagName() == "database")
    {
      ...
      if(!ok)
        qDebug(mdb.lastError().toLatin1().data());
    }
    else
    {
