...
// Do some simple processing used by both Background and Watermark
        ...
        bool doBgWm = false;
        int printMarginWidth = _printer->margins().width();
        int printMarginHeight = _printer->margins().height();
        ...
        if(pageWidth < 1 || pageHeight < 1) {
          ...
          pageWidth = (int)(((_painter->viewport().width() + printMarginWidth + printMarginWidth) / xDpi) * resolution);
          pageHeight = (int)(((_painter->viewport().height() + printMarginHeight + printMarginHeight) / yDpi) * resolution);
        }
...
// Render Watermark
        ...
        if(doBgWm)
        {
          QRectF target(-printMarginWidth, -printMarginHeight, (_painter->viewport().width() + printMarginWidth + printMarginWidth), (_painter->viewport().height() + printMarginHeight + printMarginHeight));
          QRectF source(0, 0, image.width(), image.height());
          _painter->drawImage(target, image, source);
        }
      }
...
// Render Page Objects
      for(int i = 0; i < p->primitives(); i++)
      {
        ...
        else if(prim->type() == OROLine::Line)
        {
          ...
          pen.setWidthF((ln->weight() / 100) * _printer->resolution());

          _painter->save();
          _painter->setPen(pen);
          _painter->drawLine(QLineF(s.x() * xDpi, s.y() * yDpi, e.x() * xDpi, e.y() * yDpi));
          _painter->restore();
        }
...
void renderBackground(QImage & dest, const QImage & bgImage, const QRect & bgRect, bool bgScale, Qt::AspectRatioMode bgScaleMode, int bgAlign, unsigned int bgOpacity)
{
  ...
  if(bgScale)
    img = img.smoothScale(bgRect.width(), bgRect.height(), bgScaleMode);
