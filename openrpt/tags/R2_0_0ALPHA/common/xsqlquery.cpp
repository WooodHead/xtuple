...
bool XSqlQuery::first()
{
  if (QSqlQuery::first())
  {
    if (_data)
    {
      ...
      resetSubTotals();
      for(QMapIterator<QString,double> mit(_data->_fieldTotals); mit.hasNext(); mit.next())
      {
...
bool XSqlQuery::prev()
{
  ...
  for(; mit.hasNext(); mit.next())
  {
    delta[mit.key()] = value(mit.key()).toDouble();
  }
  if (QSqlQuery::prev())
  {
    for(mit = delta; mit.hasNext(); mit.next())
    {
...
void XSqlQuery::resetSubTotals()
{
  if (_data)
  {
    ...
    for (QMapIterator<QString,double> mit(_data->_fieldSubTotals); mit.hasNext(); mit.next())
