...
bool tGraph::drawBars() {
    ...
    for(QMapIterator<int, ORStyleData> mit(_setStyle); mit.hasNext(); mit.next()) {
...
void tGraph::draw(QPainter & paint) {
    ...
    for(QMapIterator<int, GReference> dlit(_data); dlit.hasNext(); dlit.next()) {
    ...
    if(ref_cnt > 0) {
        ...
        for(QMapIterator<int, GReference> rit(_data); rit.hasNext(); rit.next() ) {
            ...
            if(drawBars() == TRUE) {
                ...
                for(sit = ref.second; sit.hasNext(); sit.next() ) {
                ...
                for(QMapIterator<double, TSetValue> it(sort_map); it.hasNext(); it.next()) {
                    tval = it.value();
                    if(tval.second != 0.0) {
                        if(tval.second < 0) {
...
void renderGraph(QPainter & paint, const QRect & rect, ORGraphData & gData, XSqlQuery * query, const QMap<QString, QColor> & _colorMap) {
    Q3PaintDeviceMetrics dpm(paint.device());
