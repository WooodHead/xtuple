...
qreal ORPreRenderPrivate::renderSection(const ORSectionData & sectionData)
{
  ...
  for(int it = 0; it < sectionData.objects.size(); ++it)
  {
    ...
    if (elemThis->isLabel())
    {
      ...
      tb->setSize(size);
      tb->setFont(l->font);
      tb->setText(qApp->translate(_reportData->name.toAscii().data(), l->string.toAscii().data()));;
      tb->setFlags(l->align);
    ...
    for(int it = 0; it < sectionData.objects.size(); ++it)
    {
      ...
      if (elemThis->isText())
      {
        ...
        if (qstrValue.length())
        {
          ...
          int sizeLimit = _maxHeight - _bottomMargin - finishCurPageSize((l+1 == qs));

          while(qstrValue.length())
          {
            // Do we need to go to the next page for this section
            if ( (intBaseTop + (intLineCounter * intRectHeight) + intRectHeight) >= sizeLimit )
            {
              if(l > 0 && _detailQuery)
                _detailQuery->prev();
              createNewPage();
