...
void ImportWindow::helpAbout()
{
  QMessageBox::about(this, tr("MetaSQL Import Tool"),
    tr("%1 version %2"
       "\n\n%3 is a tool for importing MetaSQL files into a database."
...
void ImportWindow::sAdd()
{
  ...
  if(!files.isEmpty())
    for(QStringList::Iterator it = files.begin(); it != files.end(); ++it)
      _reports->insertItem(new Q3ListBoxText(*it));
}
      ...
      if(!xml_file.isEmpty())
      {
        ...
        if(file.open(QIODevice::ReadOnly))
        {
          ...
          if(!contents.isEmpty())
          {
            ...
            QStringList lines  = metasql_src.split("\n");
            ...
            if(!metasql_name.isEmpty() && !metasql_group.isEmpty())
            {
              ...
              query.bindValue(":system",QVariant(false, 0)); 
              if(!query.exec())
              {
                err = query.lastError();
