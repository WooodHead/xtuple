...
#define MSG_BUFFER_LENGTH 1024
    ...
    if(rptLogFile == 0) {
        ...
        if(rptLogFile != 0) {
            if(!rptLogFile->open(QIODevice::WriteOnly | QIODevice::Append)) {
                delete rptLogFile;
                rptLogFile = 0;
...
class orReportPrivate {
    public:
        ...
        double _scale;
        bool _printPage;   // should we actually be printing the current page
                           // used for the fromPage() toPage() support

        Q3PtrList<orQuery> _lstQueries;
...
{
    if(_pageCounter > 0 && _printPage) {
        finishCurPage(pPainter, pPrinter);
        if (pPrinter != 0 && ((pPrinter->toPage() == 0) || (pPrinter->toPage() >= (_pageCounter + 1))))
        {
            pPrinter->newPage();
            pPainter->resetMatrix();
            pPainter->scale(_scale, _scale);
        }
    }
    ...
    if(pPrinter != 0)
    {
      if(pPrinter->fromPage() == 0 && pPrinter->toPage() == 0)
        _printPage = true;
      else if(pPrinter->toPage() == 0 && pPrinter->fromPage() <= _pageCounter)
        _printPage = true;
      else if(pPrinter->fromPage() <= _pageCounter && pPrinter->toPage() >= _pageCounter)
        _printPage = true;
      else
        _printPage = false;
    }
...
int orReportPrivate::renderSection(const ORSectionData & sectionData, QPainter * pPainter, QPrinter *) {
    ...
    int dpi = 72;
    if(pPainter != 0) {
        Q3PaintDeviceMetrics pdm(pPainter->device());
        dpi = pdm.logicalDpiX();
    }
    ...
    while( (elemThis = it.current()) != 0) {
        ...
        if (elemThis->isLabel()) {
            ...
            if(f->trackTotal) {
                ...
                if(f->format.length() > 0 && xqry) {
                    ...
                    if(!f->builtinFormat) {
						...
						if ((_database.driverName() != "QOCI8") && (_database.driverName() != "QOCI"))	// MANU
                        	str = QString().sprintf(getSqlFromTag("fmt02",_database.driverName()),(const char*)getFunctionFromTag(f->format), d_val);
						else
              				str = QString().sprintf(getSqlFromTag("fmt02",_database.driverName()), d_val);
            ...
            if(pPainter != 0 && _printPage) {
                if(im->mode == "stretch") {
                    img = img.smoothScale(rect.width(), rect.height(), Qt::KeepAspectRatio);
                }
                QPixmap pmap(img);
...
void orReport::constructor(const QString &pReportName)
{
  ...
  report.prepare( getSqlFromTag("fmt04", _database.driverName()));		// MANU
...
bool orReport::print(QPrinter *prtThis, bool boolSetupPrinter) {
  ...
  if (multiPainter) {
    ...
    {
#ifdef Q_WS_MAC
      multiPrinter->setOrientation((_internal->_reportData->page.isPortrait() ?
	...
	retval = multiPrinter->newPage();
      if (retval)
      {
	_internal->_scale = ((double)multiPrinter->resolution())/100.0;
	if(multiPrinter->outputFormat() != QPrinter::PdfFormat)
	{
	  multiPainter->resetMatrix();
	  multiPainter->scale(_internal->_scale, _internal->_scale);
	}
    ...
    if(_internal != 0) {
        ...
        if(_internal->_valid && _internal->_reportData) {
...
#ifdef Q_WS_MAC
            _internal->_pdfPrint = (prtThis->outputFormat() == QPrinter::PdfFormat);
            if(prtThis->outputFormat() == QPrinter::PdfFormat)
#endif
              prtThis->setResolution(100); // 100dpi
            if (boolSetupPrinter) {
                prtThis->setMinMax(1, 9999);
#ifdef Q_WS_MAC
            ...
            if(retval == true) {
                ...
                _internal->_scale = ((double)prtThis->resolution())/100.0;
                if(prtThis->outputFormat() != QPrinter::PdfFormat)
                  pntPrinter.scale(_internal->_scale, _internal->_scale);
...
//    QMatrix::setTransformationMode( QMatrix::Areas );
#endif
    ...
    if(_internal != 0) {
        ...
        if(_internal->_valid && _internal->_reportData != 0) {
            ...
            if(retval != false) {
                ...
                // adjust the values so that elements are printed in the correct
                // positions regardless of what our actual printing area is
                if(pPrinter != 0) {
// hack to push Mac printing up and to the left just under 1/16 of an inch
#ifdef Q_WS_MAC
#define OFFSET 6
#else
#define OFFSET 0
#endif
                    _internal->_leftMargin -= pPrinter->margins().width() + OFFSET;
                    _internal->_topMargin -= pPrinter->margins().height() + OFFSET;
                    _internal->_maxHeight -= pPrinter->margins().height() + OFFSET;
                }
                ...
                QString tQuery = getSqlFromTag("fmt03", _internal->_database.driverName());	// MANU
                ...
                int ii = 0;
                for(ii = 0; ii < _internal->_reportData->trackTotal.count(); ii++) {
                    ...
                    _internal->_subtotPageCheckPoints.insert(_internal->_reportData->trackTotal[ii], 0);

                    XSqlQuery * xqry = _internal->getQuerySource(_internal->_reportData->trackTotal[ii].query)->getQuery();
                    if(xqry) xqry->trackFieldTotal(_internal->_reportData->trackTotal[ii].column);
                }
                                        ...
                                        if(col >= numCols) {
                                            ...
                                            if(row >= numRows) {
                                                ...
                                                if(pPrinter) {
                                                    pPrinter->newPage();
                                                    pPainter->resetMatrix();
                                                    pPainter->scale(_internal->_scale, _internal->_scale);
                                                }
                                            }
                                        }
                                    } while(query->next());
