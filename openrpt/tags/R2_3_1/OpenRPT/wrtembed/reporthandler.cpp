  ...
  : QObject(parent, name)
{
  ...
  fileNewAction = new QAction( QIcon(":/OpenRPT/icons/document_32.png"),
                              tr("&New File"),QKeySequence::New,this,"file new");
  fileOpenAction = new QAction( QIcon(":/OpenRPT/icons/files_zoom_32.png"),
                               tr("&Open File..."),QKeySequence::Open,this,"file open");
  fileSaveAction = new QAction( QIcon(":/OpenRPT/icons/diskette_32.png"),
                               tr("&Save File"),QKeySequence::Save,grpDoc,"file save");
  fileSaveAsAction = new QAction( tr("Save &As..."),0,grpDoc,"file save as");
  fileCloseAction = new QAction( tr("&Close"),QKeySequence::Close,grpDoc,"file close");
  filePreviewAction = new QAction(tr("Print Preview..."), 0, grpDoc, "file print preview");
  filePrintAction = new QAction(tr("Print..."), QKeySequence::Print, grpDoc, "file print");
  filePrintToPDFAction = new QAction(tr("Print to PDF..."), 0, grpDoc, "file print to pdf");
  ...
  gridShowAction = new QAction( QIcon(":/OpenRPT/icons/numeric_field_32.png"),
                               tr("Show Grid"), 0, grpGrid, "grid show");
  ...
  gridSnapAction = new QAction( QIcon(":/OpenRPT/icons/numeric_field_ok_32.png"),
                               tr("Snap to Grid"), 0, grpGrid, "grid snap");
  ...
  itemLabelAction = new QAction( QIcon(":/OpenRPT/icons/attribute_32.png"),
                                tr("Insert Label"), 0, grpItem, "item label");
  itemFieldAction = new QAction( QIcon(":/OpenRPT/icons/filed_32.png"),
                                tr("Insert Field"), 0, grpItem, "item field");
  itemTextAction = new QAction( QIcon(":/OpenRPT/icons/text_field_32.png"),
                               tr("Insert Text"), 0, grpItem, "item text");
  itemLineAction = new QAction( QIcon(":/OpenRPT/icons/line_32.png"),
                               tr("Insert Line"), 0, grpItem, "item line");
  itemRectAction = new QAction( QIcon(":/OpenRPT/icons/box_32.png"),
                                tr("Insert Rectangle"), 0, grpItem, "item rect");
  itemBarcodeAction = new QAction( QIcon(":/OpenRPT/icons/barcode_32.png"),
                                  tr("Insert Bar Code"), 0, grpItem, "item barcode");
  itemImageAction = new QAction( QIcon(":/OpenRPT/icons/image_field_32.png"),
                                tr("Insert Image"), 0, grpItem, "item image");
  itemGraphAction = new QAction( QIcon(":/OpenRPT/icons/statistic_32.png"),
                                tr("Insert Chart/Graph"), 0, grpItem, "item graph");

  dbConnectAction = new QAction( _strMenuConnect, 0, this, "db connect");
  ...
  docTitleAction           = new QAction(tr("Properties..."),         0, grpDoc, "doc title");
  docPageSetupAction       = new QAction(tr("&Page Setup..."),        0, grpDoc, "doc page setup");
  docQuerySourceListAction = new QAction(tr("Query &Sources..."),     0, grpDoc, "doc query source list");
  docSectionEditorAction   = new QAction(tr("Section Editor..."),     0, grpDoc, "doc section editor");
  docColorListAction       = new QAction(tr("Color Definitions..."),  0, grpDoc, "doc color definitions");
  docDefinedParamsAction   = new QAction(tr("Defined Parameters..."), 0, grpDoc, "doc defined params");
...
QAction * ReportHandler::populateMenuBar(QMenuBar * menubar, QAction * exitAction)
{
  ...
  filePreviewAction->addTo(mFile);
  filePrintAction->addTo(mFile);
  filePrintToPDFAction->addTo(mFile);
  ...
  docTitleAction->addTo(mDoc);
  docPageSetupAction->addTo(mDoc);
  docQuerySourceListAction->addTo(mDoc);
  docSectionEditorAction->addTo(mDoc);
  docColorListAction->addTo(mDoc);
  docDefinedParamsAction->addTo(mDoc);
    ...
    for(int i = 0; i < nlist.count(); i++ ) {
        ...
        if(it.isElement()) {
            ...
            else if(n == "database")
            {
                ...
                MemDbLoader mdb;
                bool ok = mdb.load(_databaseElt);
                if(!ok)
                    QMessageBox::warning(rw, rw->filename, mdb.lastError());
            } else {
...
void ReportHandler::dbConnect() {
    ...
    bool dbConnected = (dbConnectAction->menuText() == _strMenuDisconnect);
...
ReportWindow * ReportHandler::activeReportWindow()
{
  ...
  if(_parentWindowIsWorkspace)
  {
    ...
    for (i = list.size() - 1; i >= 0; i--)
      if (list.at(i)->inherits("ReportWindow"))
        break;
...
void ReportHandler::dbMenuAboutToShow() {
    ...
    if(db.isValid() && db.isOpen() && _databaseElt.isNull()) {
        ...
        dbConnectAction->setMenuText(_strMenuDisconnect);
    } else {
        dbLoadAction->setEnabled(false);
        dbSaveAction->setEnabled(false);
        dbConnectAction->setMenuText(_strMenuConnect);
    }
}
...
void ReportHandler::print(bool showPreview)
{
    ...
    ORPreRender pre;
    pre.setDom(rw->document());

    ParameterEdit dlgEdit(rw);
    if(dlgEdit.setDocument(rw->document()))
    {
    ...
    if(doc)
    {
        ...
        if(showPreview) 
        {
            PreviewDialog preview (doc, &printer, rw);
...
void ReportHandler::filePrintToPDF()
{
    ...
    QString outfile = QFileDialog::getSaveFileName( rw, tr("Choose filename to save"), tr("print.pdf"), tr("Pdf (*.pdf)") );
...
{
  return QObject::tr("Copyright (c) 2002-2008, OpenMFG, LLC.");
}
