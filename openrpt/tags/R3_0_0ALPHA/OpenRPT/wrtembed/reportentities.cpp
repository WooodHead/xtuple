...
void ReportEntity::buildXML(Q3CanvasItem * item, QDomDocument & doc, QDomElement & parent) {
...
void ReportEntity::drawSelection(QRect rect, QPainter& painter)
{
	...
	int halfH = (int)(rect.height() / 2);
	QPoint center = rect.center();
	painter.fillRect(center.x()-(halfW+2), center.y()-(halfH+2), 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()-2, center.y()-(halfH+2), 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()+(halfW-2), center.y()-(halfH+2), 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()+(halfW-2), center.y()-2, 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()+(halfW-2), center.y()+(halfH-2), 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()-2, center.y()+(halfH-2), 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()-(halfW+2), center.y()+(halfH-2), 5, 5, QColor(128, 128, 255));
	painter.fillRect(center.x()-(halfW+2), center.y()-2, 5, 5, QColor(128, 128, 255));
...
ReportEntityLabel::ReportEntityLabel(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), txt(QObject::tr("Label")), flags(0) {
    fnt = getDefaultEntityFont();
    QRect r = getTextRect();
    setSize(r.width()-1, r.height()-1);
}

ReportEntityLabel::ReportEntityLabel(const QString & t, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), txt(t), flags(0) {
    fnt = getDefaultEntityFont();
    QRect r = getTextRect();
    setSize(r.width()-1, r.height()-1);
}

ReportEntityLabel::ReportEntityLabel(const QString & t, QFont f, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), txt(t), flags(0), fnt(f) {
    QRect r = getTextRect();
    setSize(r.width()-1, r.height()-1);
}
ReportEntityLabel::ReportEntityLabel(QDomNode & element, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), flags(0) {
    fnt = getDefaultEntityFont();
        ...
        if(n == "string") {
            ...
            for(int ri = 0; ri < rnl.count(); ri++) {
                ...
                if(n == "x") {
                    ...
                    d = ((node.firstChild().nodeValue().toFloat()) / 100.0) * dpiX;
                    setSize(roundDouble(d),height());
                } else if(n == "height") {
                    d = ((node.firstChild().nodeValue().toFloat()) / 100.0) * dpiY;
                    setSize(width(),roundDouble(d));
                } else {
...
void ReportEntityLabel::propertyDialog(QMap<QString,QColor>*, QuerySourceList*, QWidget * parent) {
    ...
    if(le->exec() == QDialog::Accepted) {
        ...
        dx = dx * dpiX;
        int ix = roundDouble(dx);
        if(rect().x() != ix) {
            setX(ix);
            if(_rw) _rw->setModified(TRUE);
        }
        dy = dy * dpiY;
        int iy = roundDouble(dy);
        if(rect().y() != iy) {
            setY(iy);
            if(_rw) _rw->setModified(TRUE);
        }
        dw = dw * dpiX;
        dh = dh * dpiY;
        int iw = roundDouble(dw);
        int ih = roundDouble(dh);
...
//
// methods (constructors)
ReportEntityField::ReportEntityField(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), qry(QString::null), flags(0) {
    fnt = getDefaultEntityFont();
    _trackTotal = FALSE;
...
ReportEntityField::ReportEntityField(const QString & s, const QString & c, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), qry(s), clmn(c), flags(0) {
    fnt = getDefaultEntityFont();
    _trackTotal = FALSE;
    _trackBuiltinFormat = FALSE;
    _useSubTotal = FALSE;
...
ReportEntityField::ReportEntityField(const QString & s, const QString & c, QFont f, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), qry(s), clmn(c), flags(0), fnt(f) {
    _trackTotal = FALSE;
    _trackBuiltinFormat = FALSE;
    _useSubTotal = FALSE;
    _trackTotalFormat = QString::null;
...
ReportEntityField::ReportEntityField(QDomNode & element, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), flags(0) {
    fnt = getDefaultEntityFont();
    _trackTotal = FALSE;
    _trackBuiltinFormat = FALSE;
...
void ReportEntityField::setTextFlags(Qt::Alignment alignment, bool wordwrap) {
  setTextFlags(wordwrap ? (Qt::TextWordWrap | alignment) : (int)alignment);
}
...
void ReportEntityField::drawShape(QPainter & painter) {
    ...
    painter.setFont(fnt);
    painter.setPen(col);
    painter.drawText(rect(), flags, clmn+QObject::tr(":")+qry+(_trackTotal? QObject::tr(" field total"):QObject::tr(" field")));
...
void ReportEntityField::propertyDialog(QMap<QString,QColor>*, QuerySourceList * qsl, QWidget * parent) {
    ...
    if(le->exec() == QDialog::Accepted) {
        ...
        setTextFlags(le->labelPreview->alignment(), le->labelPreview->wordWrap());
        canvas()->setChanged(rect());
        double dt;
        bool ok;
        dt = le->leXPos->text().toDouble(&ok);
...
//
// methods (constructors)
ReportEntityText::ReportEntityText(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), flags(0), bpad(0.0) {
    fnt = getDefaultEntityFont();
    QRect r = getTextRect();
    setSize(r.width()-1, r.height()-1);
}

ReportEntityText::ReportEntityText(const QString & s, const QString & c, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), qry(s), clmn(c), flags(0), bpad(0.0) {
    fnt = getDefaultEntityFont();
    QRect r = getTextRect();
    setSize(r.width()-1, r.height()-1);
}

ReportEntityText::ReportEntityText(const QString & s, const QString & c, QFont f, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), qry(s), clmn(c), flags(0), fnt(f), bpad(0.0) {
    QRect r = getTextRect();
    setSize(r.width()-1, r.height()-1);
}
ReportEntityText::ReportEntityText(QDomNode & element, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), flags(0), bpad(0.0) {
    fnt = getDefaultEntityFont();
    QDomNodeList nl = element.childNodes();
    QString n;
...
//
ReportEntityLine::ReportEntityLine(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasLine(canvas), _rw(rw) {
}
ReportEntityLine::ReportEntityLine(QDomNode & entity, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasLine(canvas), _rw(rw) {
     ...
     setPoints(roundDouble(sx), roundDouble(sy), roundDouble(ex), roundDouble(ey));
}

unsigned int ReportEntityLine::weight() const { return pen().width(); }
void ReportEntityLine::setWeight(int w) {
    if(pen().width() != w) {
        QPen p = pen();
        p.setWidth(w);
        setPen(p);
...
//
// class ReportEntityRect
//
ReportEntityRect::ReportEntityRect(ReportWindow * rw, Q3Canvas * canvas)
: Q3CanvasRectangle(canvas), _rw(rw) {
}
ReportEntityRect::ReportEntityRect(QDomNode & entity, ReportWindow * rw, Q3Canvas * canvas)
: Q3CanvasRectangle(canvas), _rw(rw) {
    // parse the xml entity we have been passed
    QDomNodeList nl = entity.childNodes();
    QDomNode node;
    ...
    for(int i = 0; i < nl.count(); i++) {
        ...
        if(n == "weight") {
            ...
            witdh = ((node.firstChild().nodeValue().toDouble()) / 100.0) * dpiX;
        } else if(n == "height") {
            height = ((node.firstChild().nodeValue().toDouble()) / 100.0) * dpiY;
        } else {
            qDebug("While parsing line encountered unknown element: %s", n.latin1());
        }
    }
    setX(roundDouble(sx));
    setY(roundDouble(sy));
    setSize (roundDouble(witdh), roundDouble(height));
}

unsigned int ReportEntityRect::weight() const { return pen().width(); }
void ReportEntityRect::setWeight(int w) {
    if(pen().width() != w) {
        QPen p = pen();
        p.setWidth(w);
        setPen(p);
...
void ReportEntityRect::drawShape(QPainter & painter) {
    ...
    if(isSelected()) {
        ...
        // draw a selected border for visual purposes
        painter.setPen(QPen(QColor(128,128,255), 0, Qt::DotLine));
        painter.drawRect((int)(x()-2), (int)(y()-2), 5, 5);
        painter.drawRect((int)(x()+width()-2), (int)(y()+height()-2), 5, 5);

        painter.setPen(p);
    }
}

void ReportEntityRect::propertyDialog(QMap<QString,QColor>*, QuerySourceList*, QWidget * parent) {
    // all we have for the user to edit is the line width
    // we should be able to do that with one of the Builtin QT dialogs
    // i think a spinner of some kind would work best
    bool ok;
...
void ReportEntityRect::buildXML(QDomDocument & doc, QDomElement & parent) {
    QDomElement entity = doc.createElement("rect");

    double bx, by, bwidth, bheight;

    bx = (x() / dpiX) * 100.0;
    by = (y() / dpiY) * 100.0;
    bwidth = ((double)(width()) / dpiX) * 100.0;
    bheight = ((double)(height()) / dpiY) * 100.0;

    QDomElement e;

    e = doc.createElement("x");
    e.appendChild(doc.createTextNode(QString::number((int)bx)));
    entity.appendChild(e);

    e = doc.createElement("y");
    e.appendChild(doc.createTextNode(QString::number((int)by)));
    entity.appendChild(e);

    e = doc.createElement("width");
    e.appendChild(doc.createTextNode(QString::number((int)bwidth)));
    entity.appendChild(e);

    e = doc.createElement("height");
    e.appendChild(doc.createTextNode(QString::number((int)bheight)));
    entity.appendChild(e);
...
int ReportEntityRect::rtti() const { return RTTI; }
int ReportEntityRect::RTTI = ReportEntity::EntityRect;
...
//
// methods (constructors)
ReportEntityBarcode::ReportEntityBarcode(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), frmt(QObject::tr("3of9")), _align(0), _narrowBarWidth(ORBarcodeData::defaultNarrowBarWidth()) {
    setMaxLength(5);
    //QRect r = getTextRect();
    setSize((int)(min_width_total*dpiX), (int)(min_height*dpiY));
}

ReportEntityBarcode::ReportEntityBarcode(const QString & s, const QString & c, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), qry(s), clmn(c), frmt(QObject::tr("3of9")), _align(0), _narrowBarWidth(ORBarcodeData::defaultNarrowBarWidth()) {
    setMaxLength(5);
    //QRect r = getTextRect();
    setSize((int)(min_width_total*dpiX), (int)(min_height*dpiY));
}

ReportEntityBarcode::ReportEntityBarcode(QDomNode & element, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw), frmt(QObject::tr("3of9")), _narrowBarWidth(ORBarcodeData::defaultNarrowBarWidth()) {
    setMaxLength(5);
    QDomNodeList nl = element.childNodes();
    QString n;
    QDomNode node;
...
int  ReportEntityBarcode::maxLength() const { return maxlength; }
...
int ReportEntityBarcode::alignment() const { return _align; }
...
double ReportEntityBarcode::narrowBarWidth() const { return _narrowBarWidth; }

void ReportEntityBarcode::setNarrowBarWidth(double bw) {
...
void ReportEntityBarcode::propertyDialog(QMap<QString,QColor>*, QuerySourceList * qsl, QWidget * parent) {
    ...
    double dh = (double)(rect().height())/(double)(dpiY);
    le->leHeight->setText(QString::number(dh,'g',3));
    le->leNarrowBarWidth->setText(QString::number(_narrowBarWidth,'g',3));
...
ReportEntityImage::ReportEntityImage(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw) {
    _img_inline = false;
    setSize((int)(1.0*dpiX), (int)(0.5*dpiY));
}

ReportEntityImage::ReportEntityImage(QDomNode & element, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw) {
    QDomNodeList nl = element.childNodes();
    QString n;
    QDomNode node;
...
// ReportEntityGraph
//
ReportEntityGraph::ReportEntityGraph(ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw) {
    setSize((int)(1.0*dpiX), (int)(0.5*dpiY));
...
ReportEntityGraph::ReportEntityGraph(QDomNode & element, ReportWindow * rw, Q3Canvas * canvas)
  : Q3CanvasRectangle(canvas), _rw(rw) {
    ...
    double dw = ((_graphData.rect.width()) / 100.0) * dpiX;
    double dh = ((_graphData.rect.height()) / 100.0) * dpiY;

    setSize(roundDouble(dw), roundDouble(dh));
}

ReportEntityGraph::ReportEntityGraph(ORGraphData & gData, ReportWindow * rw, Q3Canvas * canvas) :
  Q3CanvasRectangle(canvas), _rw(rw) {
    ...
    for(int i = 0; i < gData.series.count(); i++) {
