    ...
    else if (elemThis->isField())
    {
      ...
      if(f->trackTotal) {
        ...
        if(f->format.length() > 0 && xqry)
        {
          ...
          if(!f->builtinFormat)
              str = QString().sprintf(f->format.toLatin1().data(), d_val);
          else
          {
			if ((_database.driverName() != "QOCI8") && (_database.driverName() != "QOCI"))		// MANU
              str = QString().sprintf(getSqlFromTag("fmt02",_database.driverName()).toLatin1().data(),getFunctionFromTag(f->format).toLatin1().data(), d_val);	// MANU
			else
              str = QString().sprintf(getSqlFromTag("fmt02",_database.driverName()).toLatin1().data(), d_val);	// MANU
...
// the graph drawing code.
    ...
    else if (elemThis->isCrossTab())
    {
      ...
      crossTab.PopulateFromQuery(getQuerySource(ctData->data.query)->getQuery());
