...
bool parseReportData(const QDomElement & elemSource, ORDataData & dataTarget)
{
  ...
  for( unsigned int paramCounter = 0; paramCounter < params.count(); paramCounter++ )
  {
...
bool parseReportColorDefData(const QDomElement & elemSource, ORColorDefData & coldefTarget)
{
  ...
  for(unsigned int nodeCounter = 0; nodeCounter < nlist.count(); nodeCounter++)
  {
      ...
      for(unsigned int i = 0; i < nl.count(); i++)
      {
        ...
        else if(node.nodeName() == "head")
        {
          ...
          if(parseReportSection(node.toElement(), *sd) == TRUE)
          {
            dgsd->head = sd;
            sectionTarget.trackTotal += sd->trackTotal;
            for(QValueListIterator<ORDataData> it = sd->trackTotal.begin(); it != sd->trackTotal.end(); ++it)
...
bool parseReport(const QDomElement & elemSource, ORReportData & reportTarget)
{
  ...
  for(unsigned int nodeCounter = 0; nodeCounter < section.count(); nodeCounter++)
  {
    ...
    {
      QString n = elemThis.attribute("name");
      QString d = elemThis.text();
      if(!n.isEmpty())
        reportTarget.definedParams.insert(n, d);
    }
