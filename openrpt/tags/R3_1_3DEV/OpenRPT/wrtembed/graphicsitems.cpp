...
void ORGraphicsRectItem::mouseReleaseEvent(QGraphicsSceneMouseEvent * event)
{
  if(scene())
  {
    ...
    ORGraphicsSectionItem * csection = static_cast<DocumentScene*>(scene())->getSection(event->scenePos());
    if(csection)
        csection->highlight(ORGraphicsSectionItem::Normal);

    if(!(event->modifiers() & Qt::ShiftModifier))
    {
        static_cast<DocumentScene*>(scene())->updateSectionsForSelectedItems();
    }
